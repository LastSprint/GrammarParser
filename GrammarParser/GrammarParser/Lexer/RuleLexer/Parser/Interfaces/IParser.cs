...
namespace GrammarParser.Lexer.RuleLexer.Parser.Interfaces {

    /// <summary>
    ///     Интерфейс для абстрактного парсера лексем, который может:
    ///     - Попытаться распарсить и вернуть null в случае неудачи
    ///     - Попытаться распарсить и выкинуть исключение если лексема задекларирована с ошибкой (эти исключения нет смысла
    ///     перехватывать - они дял пользователей. ДАльнейший парсинг в таком случае невозможен)
    ...
    public interface IParser {
        ...
        ///     (Так как у нас тут не Nullsafety то с определенной долей везения можно утверждать, что IParser.Parse() вернет null,
